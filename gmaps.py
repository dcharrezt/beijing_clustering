...
def scatter(latitudes, longitudes, colors=None, maptype=MAPTYPE):
    ...
    plt.savefig(title +'_scatter.png', bbox_inches='tight', w_pad=-1, h_pad=-1)
#    plt.show()
...
def heatmap(latitudes, longitudes, values, resolution=None, ms=0.15,maptype=MAPTYPE):
    ...
    plt.figure(figsize=(10, 10))
    plt.imshow(np.array(img))                                               # Background map
    plt.imshow(z, origin='lower', extent=[0, width, 0, width], alpha=ms)  # Foreground, transparent heatmap
    ...
    plt.tight_layout()
    plt.subplots_adjust(left=10, bottom=10, right=20, top=20, wspace=None, hspace=None)
    plt.savefig(title +'_heat.png', bbox_inches='tight', w_pad=-1, h_pad=-1)
#    plt.show()
...
def polygons(latitudes, longitudes, clusters, title, maptype=MAPTYPE):
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.subplots_adjust(left=10, bottom=10, right=20, top=20,
                    wspace=None, hspace=None)
    plt.savefig(title +'_poly.png', bbox_inches='tight', w_pad=-1, h_pad=-1)
#    plt.show()
